...
public class ImgCreator {
    ...
    public void process(File pdf, File output, int resolution, int width, int height, ImageFileExtension extension) {
        log.info("starting images conversion into {} from {}", output, pdf);
    ...
    private void persistImage(BufferedImage image, int width, int height, File output) throws IOException {
        log.info("persisting image `{}`", output.getAbsolutePath());
        final File parentFile = output.getParentFile();
        final boolean madeDirs = parentFile.mkdirs();
        if (!madeDirs && !parentFile.exists()) {
            log.warn("failed to make dirs for {}", parentFile);
            throw new IllegalStateException("failed to make dirs " + parentFile);
        }
